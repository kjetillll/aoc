...
my $modulus = eval join '*', map $$_{div_by}, @monkey;
say "Modulus: $modulus";
    
for my $round (1..10000){
    print "Round $round\r";
    my $devnull; open local *STDOUT, '>', \$devnull if $round<10000;
    for my $i (0 .. $#monkey){
        say "Monkey $i:";
        my $m = $monkey[$i];
        while( @{ $$m{items} } ){
    ...
    say "== After round 10000 ==";
    say "Monkey $_ ispected items $monkey[$_]{inspected} times." for 0 .. $#monkey;
}

say "Answer: ", eval join '*', (sort {$b<=>$a} map $$_{inspected}, @monkey)[0,1];

#time perl 2022_day_11_part_2.pl 2022_day_11_input.txt | tail   # 3.50 sec
#Answer: 13937702909
